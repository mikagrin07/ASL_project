...
def define_global_variables():
    global g_forced_num_classes
    g_forced_num_classes = 26   
...
def Y_to_one_hot(Y):
    '''global g_forced_num_classes
    num_classes = g_forced_num_classes if g_forced_num_classes != 0 else 26
    one_hot_Y = np.zeros((Y.shape[0], num_classes))
    
    if g_forced_num_classes != 0:
        mask = (Y == 0) | (Y == 1)
        indices = np.where(mask, Y, 2)
        indices = np.clip(indices, 0, num_classes - 1)
        one_hot_Y[np.arange(Y.shape[0]), indices] = 1
    else:
